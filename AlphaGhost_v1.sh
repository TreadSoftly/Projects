...
function reboot() {
    # Check and install required packages
    declare -a packages=("macchanger" "anonsurf")
    for package in "${packages[@]}"; do
        if ! dpkg-query -W -f='${Status}' "$package" >/dev/null 2>&1 | grep -q "install ok installed"; then
    ...
    # Set GRUB timeout to 0 if it's not already set
    if ! grep -q "GRUB_TIMEOUT=0" /etc/default/grub; then
        sed -i 's/GRUB_TIMEOUT=[[:digit:]]\+/GRUB_TIMEOUT=0/' /etc/default/grub
        update-grub
    fi
    ...
    # Default to eth0 if the network interface is not detected
    if [ -z "$interface" ]; then
        interface="eth0"
    fi
    ...
    # Replace the MAC address with the original MAC address
...
function endit() {
    # Check and install required packages
    declare -a packages=("macchanger" "bleachbit")
    for package in "${packages[@]}"; do
        if ! dpkg-query -W -f='${Status}' "$package" >/dev/null 2>&1 | grep -q "install ok installed"; then
            apt-get -qq install -y "$package" >/dev/null 2>&1
    ...
    if [ -n "$interface" ]; then
        ...
        printf "${RED}Error: Network interface not found.${NC}\n"
    fi

    # Check if bleachbit is installed and run it
    if command -v bleachbit &> /dev/null; then
        bleachbit --clean &
        bleachbit_pid=$!
        wait $bleachbit_pid
    else
        printf "${RED}Error: bleachbit not found.${NC}\n"
    fi
